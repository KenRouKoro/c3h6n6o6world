package com.foxapplication.mc.c3h6n6o6world.thread;
...
import static com.foxapplication.mc.c3h6n6o6world.thread.ServerUtil.LogErrorUtil;
...
public class CalculationController {
    ...
    static public void Init(){
        ...
        ForkJoinPool.ForkJoinWorkerThreadFactory poolThreadFactory = p -> {
            ...
            poolThread.setContextClassLoader(C3H6N6O6World.class.getClassLoader());
            return poolThread;
        };
    ...
    public static void callWorldTick(ServerWorld world, BooleanSupplier shouldKeepTicking){
        if (C3H6N6O6World.getSetting().getBool("WorldDisabled",false)){
            world.tick(shouldKeepTicking);
            return;
        }
        //判断是否处于tick中，不处于直接tick
        if(!Ticking.get()){
            world.tick(shouldKeepTicking);
            return;
        }
        ...
        executor.execute(() -> {
            try{
                ...
                if(C3H6N6O6World.getSetting().getBool("logEntityException",true)){
                    log.warn("黑索金捕捉到在世界Tick:" + Thread.currentThread().getName() + "有单独的空指针异常，可能是实体出了点小问题，如果有假死的实体，重启服务器，大概率能解决。");
                    LogErrorUtil(e);
                }
            }catch(IllegalArgumentException e){
                if(C3H6N6O6World.getSetting().getBool("logEntityException",true)) {
                    log.warn("黑索金捕捉到在世界Tick:" + Thread.currentThread().getName() + "有线程安全报错。");
                    LogErrorUtil(e);
                }
            } catch (Exception e){
                if(C3H6N6O6World.getSetting().getBool("logEntityException",true)) {
                    String eMessage = e.getMessage();
                    log.error("黑索金捕捉到在世界Tick:" + Thread.currentThread().getName() + " 抛出异常:" + e.getClass().getName() + ":" + eMessage + "\n");
                    LogErrorUtil(e);
                    CrashReport crashReport = CrashReport.create(e, "Exception ticking world");
    ...
    public static void callEntityTick(Entity entityIn) {
        if (C3H6N6O6World.getSetting().getBool("EntityDisabled",false)){
            entityIn.tick();
            return;
        }
    ...
    public static void callBlockEntityTick(BlockEntityTickInvoker blockEntityTick) {
        if (C3H6N6O6World.getSetting().getBool("BlockEntityDisabled",false)) {
        ...
        executor.execute(() -> {
            try {
                ...
                if (C3H6N6O6World.getSetting().getBool("logBlockEntityRuntimeException", false)) {
                    log.error("黑索金捕捉到在方块实体tick:" + Thread.currentThread().getName() + "发生运行时异常，异常点位于：" + blockEntityTick.getPos().toString());
                    LogErrorUtil(e);
                }
            } catch (Exception e) {
                if (C3H6N6O6World.getSetting().getBool("logBlockEntityException", true)) {
